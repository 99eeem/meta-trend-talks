    ...
    <div className={styles.articleList}>
        {currentItems.map((item: any, index: number) => (
            <div className={styles.categoryNews} key={item.id}>
                <NewsArticle date={item.createdAt} title={item.title} id={item.id} category={item.category.name} thumbnail={item.thumbnail.url} author={item.author} body={item.body} isShowImage={true}/>
         </div>))}
