    ...
    <div className={styles.headerContents}>
      <Link href="/" className={styles.logoContainer}>
        <div className={styles.logoBox}>
          <img src="/images/logoWithBlackName.png" alt={`ロゴの画像`} />
        </div>
      </Link>
      <div className={styles.menu}>
        {!isSearchVisible && (
          <ul className={styles.headerCategory}>
            {props.contenst.map((item: any, index: number) => (
              <Link key={index} href={`/news/${item.link}`}>
                <li>{item.name}</li>
              </Link>
            ))}
          </ul>
        ...
        {!isSearchVisible && (
          <div className={`${styles.menuItem} ${styles.categoryMenu}`} onClick={toggleModal}>
            {isModalOpen ? <FiArrowRightCircle /> : <FiMenu />}
          </div>
        )}
      </div>
      <div className={`${styles.modal} ${isModalOpen ? styles.active : ''}`}>
        <ul className={styles.categoryList}>
          {props.contenst.map((item: any, index: number) => (
            <Link key={index} href={`/news/${item.link}`} onClick={toggleModal}>
              <li>{item.name}</li>
            </Link>
          ))}
        </ul>
